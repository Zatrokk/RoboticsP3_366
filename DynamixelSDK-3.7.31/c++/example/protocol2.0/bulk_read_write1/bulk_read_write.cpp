...
#define DXL_STARTING_VELOCITY_VALUE     50                  // Value of the dynamixel velocity
...
int getch();
int kbhit(void);
bool successTest(bool, int);
bool resultPrint(int, uint8_t, dynamixel::PacketHandler*);
...
int main()
{
    ...
    // Open port
    if (!successTest(portHandler->openPort(), 1))
        return 0;

    // Set port baudrate
    if (!successTest(portHandler->setBaudRate(BAUDRATE), 2))
        return 0;

    // Enable Torque and Specify Maximum Velocity for each Dynamixel
    for (int i = 0; i < dxl_motor_amounts; i++)
    {
        if (resultPrint(packetHandler->write1ByteTxRx(portHandler, i+1, ADDR_PRO_TORQUE_ENABLE, TORQUE_ENABLE, &dxl_error), dxl_error, packetHandler))
            printf("Dynamixel#%d has been successfully connected \n", i+1);
        resultPrint(dxl_comm_result = packetHandler->write4ByteTxRx(portHandler, i+1, ADDR_PRO_PROFILE_VELOCITY, DXL_STARTING_VELOCITY_VALUE, &dxl_error), dxl_error, packetHandler);
    }
    ...
    for (int i = 0; i < dxl_motor_amounts; i++)
    {
        resultPrint(packetHandler->write1ByteTxRx(portHandler, i + 1, ADDR_PRO_TORQUE_ENABLE, TORQUE_DISABLE, &dxl_error), dxl_error, packetHandler);
    }
...
bool successTest(bool test, int mode)
{
...
bool resultPrint(int dxl_comm_result, uint8_t dxl_error, dynamixel::PacketHandler* packetHandler)
{
    if (dxl_comm_result != COMM_SUCCESS)
    {
        printf("%s\n", packetHandler->getTxRxResult(dxl_comm_result));
